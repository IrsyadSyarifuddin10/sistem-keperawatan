<x-app-layout>
    <div class="flex sm:justify-center pt-10 sm:pt-0 bg-gray-100">
        <div class="my-6 px-6 py-4 bg-white shadow-md sm:rounded-lg">
            ...
            <form method="POST" action="{{ route('edit-data-petugas') }}">
                @csrf
                <x-text-input id="id" class="block mt-1 w-full" type="hidden" value="{{ $indexEditPetugas->id }}"
                    name="id" />
                <!-- NIP -->
                <div>
                    <x-input-label for="nip" :value="__('NIP')" />
                    <x-text-input id="nip" class="block mt-1 w-full" type="text" name="nip" :value="old('nip')" required
                        autofocus autocomplete="nip" maxlength=12 value="{{ $indexEditPetugas->nip }}" />
                ...
                <div class="mt-4">
                    ...
                    <x-text-input id="name" class="block mt-1 w-full" type="text" name="nama_petugas"
                        ...
                        value="{{ $indexEditPetugas->nama_petugas }}" />
                ...
                <div class="mt-4">
                    ...
                    <x-text-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email')"
                        required autocomplete="email" value="{{ $indexEditPetugas->email }}" />
                ...
                <div class="flex flex-row gap-4 justify-between">
                    ...
                    <div class="mt-4">
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="bk" {{ $indexEditPetugas->status === 'bk' ? 'selected' : '' }}>BK</option>
                            <option value="pk" {{ $indexEditPetugas->status === 'pk' ? 'selected' : '' }}>PK</option>
                        </select>
                    ...
                    <div class="mt-4">
                        ...
                        <select name="unit" id="unit"
                            ...
                            <option value="bk" {{ $indexEditPetugas->unit === 'bk' ? 'selected' : '' }}>BK</option>
                            <option value="ugd" {{ $indexEditPetugas->unit === 'ugd' ? 'selected' : '' }}>UGD</option>
                            <option value="rawat jalan" {{ $indexEditPetugas->unit === 'rawat jalan' ? 'selected' : ''
                                }}>Rawat Jalan
                            </option>
                            <option value="rawat inap" {{ $indexEditPetugas->unit === 'rawat inap' ? 'selected' : ''
                                }}>Rawat Inap</option>
                            <option value="perinatologi" {{ $indexEditPetugas->unit === 'perinatologi' ? 'selected' : ''
                                }}>Perina</option>
                            <option value="ok" {{ $indexEditPetugas->unit === 'ok' ? 'selected' : '' }}>OK</option>
                            <option value="icu" {{ $indexEditPetugas->unit === 'icu' ? 'selected' : '' }}>ICU</option>
                        </select>
                ...
                <div class="flex flex-row gap-4 justify-between">
                    <!-- Password -->
                    <div class="mt-4">
                        <x-input-label for="password" :value="__('Masukin Lagi Passwordmu')" />
