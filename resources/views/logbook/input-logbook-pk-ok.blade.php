...
$v_logbook_pk_ok_formatted = array_map(function ($item) {
        ...
        <div class="max-w-7xl xl:max-w-[1920px] mx-auto px-2 sm:px-2 lg:px-4">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg pt-6 px-6 ">
                <div class="p-3 text-gray-900 block w-full overflow-x-auto">
                    ...
                    <table id="tbInputLogbookPKOK" class="items-center bg-transparent w-full">
                        ...
                        <tbody>
                            <form action="{{ route('input-logbook-pk-ok') }}" method="POST">
                                <?php foreach ($v_logbook_pk_ok_formatted as $index => $item): ?>
                                <?php    $fitem = $f_logbook_pk_ok[$index] ?? null; ?>
                                <tr>
                                    <!-- Kolom Pertama: Nama logbook-pk-ok -->
                                    <td
                    ...
                    <div class="flex flex-row pt-4 px-6">
                        <x-input-label for="catatan-logbook-pk-ok" :value="__('Catatan')" />
                        <textarea name="catatan-logbook-pk-ok" class="form-control mx-2 w-full" rows="3"></textarea>
                    </div>
                    <div class="flex flex-row pt-12 justify-end">
                        ...
                        <button id="btnSimpanLogbookPKOK" type="button"
